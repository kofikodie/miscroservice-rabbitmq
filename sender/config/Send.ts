...
export class Send implements SendConfigInterface {
    async publisher(message?: string): Promise<void> {
        ...
        await channel.bindQueue(process.env.RABBITMQ_QUEUE_ONE, process.env.RABBITMQ_WORKER_EXCHANGE, '*.image.*');
        await channel.bindQueue(process.env.RABBITMQ_QUEUE_SEC, process.env.RABBITMQ_WORKER_EXCHANGE, '#.image');
        await channel.bindQueue(process.env.RABBITMQ_QUEUE_THR, process.env.RABBITMQ_WORKER_EXCHANGE, 'image.#');
        channel.publish(
